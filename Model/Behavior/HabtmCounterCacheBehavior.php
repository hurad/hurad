...
class HabtmCounterCacheBehavior extends ModelBehavior {
    ...
    function _updateCounterCache(&$model) {
        $databaseConfig = get_class_vars('DATABASE_CONFIG');
        $this->__tablePrefix = $databaseConfig['default']['prefix'];
        ...
        foreach ($this->settings[$model->alias] as $habtmAlias => $settings) {
            ...
            $updateQuery = array(
                'table' => $this->__tablePrefix . $model->$habtmAlias->table,
                'alias' => '`' . $habtmAlias . '`',
                'order' => null,
                'limit' => null,
                'group' => null,
            ...
            if ($settings['counterCache']) {
                ...
                $counterQuery = array(
                    'fields' => array('COUNT(*)'),
                    'table' => $this->__tablePrefix . $settings['joinTable'],
                ...
                if ($settings['counterScope']) {
                    ...
                    $counterQuery['joins'][] = array(
                        'type' => 'INNER',
                        'alias' => $model->alias,
                        'table' => $this->__tablePrefix . $model->table,
