<article id="post-<?php $this->Page->the_ID(); ?>" class="post-<?php $this->Page->the_ID(
    ...
    <time datetime="<?php echo h($page['Page']['created']); ?>" class="post-date"
          pubdate><?php echo $this->Time->format(
            'F jS, Y',
            $page['Page']['created'],
            null,
            Configure::read('General-timezone')
        ); ?></time>
    <h1 class="post-title"><?php echo $this->Html->link(
            $this->Page->get_the_title(),
